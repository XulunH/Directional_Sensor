...
def apply_filters(data, filters, pic_size):
    filtered_data = np.zeros((data.shape[0], pic_size, pic_size, 8)) 
...
#     filtered_data = np.zeros((data.shape[0], pic_size, pic_size, 8))
#     for i in range(data.shape[0]):
#         fig, axes = plt.subplots(1, 5, figsize=(15, 3))  # Setting up a subplot for original and 8 filters
...
model = Sequential([
    ...
    # Input(shape=(pic_size, pic_size,8*channels)),
...
epochs = 25  # Define the number of epochs here
